                    ...
                    @forelse($quizzes as $quiz)
                        <tr class="hover:bg-surface-container-low/50 transition-colors">
                            ...
                            <td class="p-md text-right space-x-2">
                                <a href="{{ route('admin.digital_learning.quizzes.questions', $quiz->id) }}" class="text-primary hover:underline font-label-md" title="Manage Questions">
                                    Questions
                                </a>
                                <a href="{{ route('admin.digital_learning.quizzes.results', $quiz->id) }}" class="text-primary hover:underline font-label-md" title="View Results">
                                    Results
                                </a>
                                <form action="{{ route('admin.digital_learning.quizzes.destroy', $quiz->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this quiz?');">
                                    @csrf @method('DELETE')
