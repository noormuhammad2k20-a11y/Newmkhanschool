                                ...
                                <tr class="hover:bg-surface-container-low transition-colors cursor-pointer group" onclick="toggleSubjects('{{ $groupKey }}')">
                                    ...
                                    <td class="py-3 px-4 text-body-md text-on-surface-variant">
                                        <div class="flex flex-col">
                                            <span class="font-medium text-on-surface"><span class="material-symbols-outlined text-[14px] align-middle mr-1">calendar_today</span>{{ $dateDisplay }}</span>
                                            <span class="text-body-sm mt-0.5"><span class="material-symbols-outlined text-[14px] align-middle mr-1">schedule</span>{{ $firstExam->exam_time }} - {{ $firstExam->end_time }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4">
                                        <button class="inline-flex items-center gap-1 bg-primary/10 hover:bg-primary/20 text-primary px-3 py-1.5 rounded-md text-label-md font-medium transition-colors border border-primary/20" onclick="event.stopPropagation(); toggleSubjects('{{ $groupKey }}')">
                                            View ({{ $group->count() }}) 
                                            <span id="icon-{{ $groupKey }}" class="material-symbols-outlined text-[18px] transition-transform duration-200">expand_more</span>
                                        </button>
                                    </td>
                                    <td class="py-3 px-4 exam-status-cell" data-subjects="{{ $subjectsData }}" data-current-status="">
                                        <div class="status-badge-container">
                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-variant text-on-surface-variant text-label-sm font-bold border border-outline-variant"><span class="material-symbols-outlined text-[14px]">hourglass_empty</span>Calculating...</span>
                                        </div>
                ...
                <div id="createStep2" class="hidden">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button type="button" id="btnGenerateTimetable" class="inline-flex items-center gap-1 px-3 py-1.5 bg-secondary/10 text-secondary hover:bg-secondary hover:text-on-secondary rounded-md transition-colors text-label-md font-medium border border-secondary/20">
                            <span class="material-symbols-outlined text-[16px]">auto_awesome</span> Generate Timetable
                        </button>
                    </div>
        ...
        document.querySelectorAll('td.exam-status-cell').forEach(cell => {
            ...
            const setInvalidStatus = (msg) => {
                ...
                if (currentBadge !== 'Invalid') {
                    ...
                    if (badgeContainer) {
                        badgeContainer.innerHTML = `<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-error/10 text-error text-label-sm font-bold border border-error/20"><span class="material-symbols-outlined text-[14px]">error</span>${msg}</span>`;
                    }
                }
                ...
                if (!sub.start || !sub.end || sub.start === '' || sub.end === '') {
                    ...
                    if (subjectCard) {
                        ...
                        if (sBadge && sBadge.getAttribute('data-current-badge') !== 'Invalid') {
                            ...
                            sBadge.innerHTML = '<span class="inline-flex items-center gap-1 text-error text-label-sm font-bold"><span class="material-symbols-outlined text-[14px]">error</span> Invalid Schedule</span>';
                        }
                    }
                    return;
                }
                    ...
                    if (badgeContainer) {
                        ...
                        if (currentBadge !== subStatus) {
                            ...
                            if (subStatus === 'Completed') {
                                badgeContainer.innerHTML = '<span class="inline-flex items-center gap-1 text-secondary text-label-sm font-bold"><span class="material-symbols-outlined text-[14px]">check</span> Completed</span>';
                                ...
                                badgeContainer.innerHTML = '<span class="inline-flex items-center gap-1 text-on-surface-variant text-label-sm font-bold"><span class="material-symbols-outlined text-[14px]">hourglass_empty</span> Upcoming</span>';
                            }
                        }
                    }
                }
            ...
            if (currentBadge !== groupStatus) {
                ...
                if (badgeContainer) {
                    if (groupStatus === 'In Progress') {
                        ...
                        badgeContainer.innerHTML = '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-secondary/10 text-secondary text-label-sm font-bold border border-secondary/20"><span class="material-symbols-outlined text-[14px]">check_circle</span>Completed</span>';
                    } else {
                        badgeContainer.innerHTML = '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-variant text-on-surface-variant text-label-sm font-bold border border-outline-variant"><span class="material-symbols-outlined text-[14px]">schedule</span>Scheduled</span>';
                    }
                }
            }
