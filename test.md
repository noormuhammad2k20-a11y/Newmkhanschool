...
<body class="bg-background text-on-background font-body-md min-h-screen flex" style="zoom: 0.8;">
    ...
    <div id="main-content" class="flex flex-col md:ml-64 w-full md:w-[calc(100%-16rem)] min-h-screen min-w-0 transition-all duration-300 ease-in-out">
        <!-- TopNavBar -->
        <!-- Premium Floating TopNavBar -->
        <div class="px-4 sm:px-6 pt-4 sticky top-0 z-30 transition-all duration-300">
            <header class="bg-surface-container-lowest/80 backdrop-blur-xl w-full h-[64px] rounded-2xl border border-outline-variant/40 shadow-sm flex justify-between items-center px-3 sm:px-4 transition-all duration-300">
                <div class="flex items-center gap-2 sm:gap-4">
                    <!-- Sidebar Toggle Button -->
                    <button id="sidebar-toggle-btn" class="w-10 h-10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-primary/5 rounded-xl transition-all duration-300 cursor-pointer active:scale-95 group">
                        <span class="material-symbols-outlined text-[22px] group-hover:scale-110 transition-transform" id="sidebar-toggle-icon">menu_open</span>
                    </button>
                    
                    <div class="h-5 w-px bg-outline-variant/40 mx-1 hidden sm:block"></div>
                    
                    <!-- Minimalist Search Bar -->
                    <div class="hidden md:flex relative group">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-on-surface-variant/70 group-focus-within:text-primary transition-colors duration-300">
                            <span class="material-symbols-outlined text-[18px]">search</span>
                        </div>
                        <input type="text" class="block w-[240px] lg:w-[360px] h-10 pl-10 pr-14 text-[13px] font-medium text-on-surface bg-transparent hover:bg-surface-container/30 focus:bg-surface-container/50 rounded-xl border-transparent focus:border-transparent focus:ring-0 transition-all duration-300 placeholder-on-surface-variant/50" placeholder="Search anywhere...">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                            <kbd class="text-[10px] font-bold text-on-surface-variant/70 bg-surface-container-lowest px-1.5 py-0.5 rounded border border-outline-variant/40 shadow-sm">⌘K</kbd>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1 sm:gap-1.5">
                    ...
                    <button class="md:hidden w-10 h-10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-primary/5 rounded-xl transition-all duration-300 cursor-pointer active:scale-95">
                        <span class="material-symbols-outlined text-[20px]">search</span>
                    </button>

                    <!-- Notifications -->
                    @if(auth()->check() && auth()->user()->role_id == 5)
                        <a href="{{ route('parent.notifications.index') }}" class="relative w-10 h-10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-surface-container rounded-xl transition-all duration-300 cursor-pointer active:scale-95 group">
                            <span class="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">notifications</span>
                            <span id="notification-badge" class="absolute top-2.5 right-2.5 w-2 h-2 border border-surface-container-lowest bg-error rounded-full hidden"></span>
                        </a>
                    @else
                        <button class="relative w-10 h-10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-surface-container rounded-xl transition-all duration-300 cursor-pointer active:scale-95 group">
                            <span class="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">notifications</span>
                            <span class="absolute top-2.5 right-2 w-1.5 h-1.5 bg-primary rounded-full ring-2 ring-surface-container-lowest hidden"></span>
                        </button>
                    @endif
                    
                    <!-- Help -->
                    <button class="w-10 h-10 hidden sm:flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-surface-container rounded-xl transition-all duration-300 cursor-pointer active:scale-95 group">
                        <span class="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform">help</span>
                    </button>
                    
                    <!-- Settings -->
                    <button class="w-10 h-10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-surface-container rounded-xl transition-all duration-300 cursor-pointer active:scale-95 group">
                        <span class="material-symbols-outlined text-[20px] group-hover:rotate-45 transition-transform duration-500">settings</span>
                    </button>

                    <div class="h-6 w-px bg-outline-variant/40 mx-2 hidden sm:block"></div>

                    <!-- Enhanced Profile Dropdown -->
                    <button class="flex items-center gap-2.5 p-1 pr-3 hover:bg-surface-container/80 rounded-full transition-all duration-300 cursor-pointer group border border-transparent hover:border-outline-variant/30">
                        <div class="w-8 h-8 rounded-full bg-secondary-container overflow-hidden ring-2 ring-transparent group-hover:ring-primary/20 transition-all duration-300 shadow-sm">
                            <img alt="Profile" class="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuB-yezbc927wY0C322XO39E8Fpj-TKkH1lavHgZS32icKx9cZkfyfH9G1q_BH780q4XWHhALV5X9kZitOylufGihq0Wd6o6gNlkskCLD8qmnDfet-MuX2Icl-w7TiaM7S4SqO5CDug81BiCvDYn6OGBKXtZw3vb2N4OG8DCquSDhDWVCxCCydotcJs0IbKIizYsoQ0nK63Dz3toqIXN5n2aaHPFI-YBQza8tPLwXkPoo5KZdewHH8OnW9C-XtWqdwDgQpT_1p3_" />
                        </div>
                        <div class="flex flex-col items-start hidden sm:flex">
                            <span class="text-[12px] font-bold text-on-surface leading-tight tracking-tight">Admin User</span>
                            <span class="text-[10px] font-medium text-primary leading-none">Management</span>
                        </div>
                        <span class="material-symbols-outlined text-[16px] text-on-surface-variant group-hover:text-primary transition-colors hidden sm:block ml-0.5">keyboard_arrow_down</span>
                    </button>
                </div>
            </header>
        </div>
